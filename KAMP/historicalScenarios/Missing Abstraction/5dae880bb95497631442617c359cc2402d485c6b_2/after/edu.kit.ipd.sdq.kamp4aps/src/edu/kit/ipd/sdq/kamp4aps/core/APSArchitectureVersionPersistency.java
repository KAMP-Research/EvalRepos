...
public class APSArchitectureVersionPersistency extends AbstractArchitectureVersionPersistency<APSArchitectureVersion> {
	...
	@Override
	public void saveModificationMarkFile(Path targetDirectoryPath, File filename, APSArchitectureVersion version)
			throws IOException {
		ResourceSet resourceSet = new ResourceSetImpl();
		String internalModFilePath = filename.getName() + "." + FILEEXTENSION_MODIFICATIONMARK;	
		if (version.getModificationMarkRepository()!=null)
			saveEmfModelToResource(version.getModificationMarkRepository(), 
					targetDirectoryPath.toAbsolutePath().toString(), internalModFilePath, resourceSet);	
		String internalIECModFilePath = filename.getName() + "." + AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_MODIFICATIONMARK;	
		if (version.getIECModificationMarkRepository()!=null)
			saveEmfModelToResource(version.getIECModificationMarkRepository(), targetDirectoryPath.toAbsolutePath().toString(), internalIECModFilePath, resourceSet);	
		String internalHMIModFilePath = filename.getName() + "." + FILEEXTENSION_HMI_MODIFICATIONMARKS;	
		if (version.getHmiModificationRepository()!=null)
			saveEmfModelToResource(version.getHmiModificationRepository(), 
					targetDirectoryPath.toAbsolutePath().toString(), internalHMIModFilePath, resourceSet);		
		
	}

	public static void savePCMAndKAMP4APSModels(Path targetDirectoryPath, File filename, APSArchitectureVersion version) {
		ResourceSet resourceSet = new ResourceSetImpl();
		String internalModFilePath = filename.getName() + "." + FILEEXTENSION_MODIFICATIONMARKS;
		String fieldOfActivityRepositoryFilePath = 
				filename.getName() + "." + FILEEXTENSION_FIELDOFACTIVITYANNOTATIONS;
		String programsFilePath = filename.getName() + "." + FILEEXTENSION_DEPLOYMENTCONTEXT;
		String xppuFilePath = filename.getName() + "." + FILEEXTENSION_APS;
		String hmiRepositoryFilePath = filename.getName() + "."+ FILEEXTENSION_HMI;
		String hmiModificationMarksFilePath = 
				filename.getName() + "."+ FILEEXTENSION_HMI_MODIFICATIONMARKS;

		if (version.getModificationMarkRepository() != null)
			saveEmfModelToResource(version.getModificationMarkRepository(), 
					targetDirectoryPath.toAbsolutePath().toString(), internalModFilePath,
					resourceSet);
		...
		if (version.getDeploymentContextRepository() != null)
			saveEmfModelToResource(version.getDeploymentContextRepository(), 
					targetDirectoryPath.toAbsolutePath().toString(), programsFilePath,
					resourceSet);
		if (version.getAPSPlant() != null)
			saveEmfModelToResource(version.getAPSPlant(), 
					targetDirectoryPath.toAbsolutePath().toString(), xppuFilePath,
					resourceSet);
		
		String repositoryfilePath = filename.getName() + "."+ AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_REPOSITORY;
		String configurationFilePath = filename.getName() + "."+ AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_CONFIGURATION;
		String internalIECModFilePath = filename.getName() + "." + AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_MODIFICATIONMARK;
		String iecFieldOfActivityRepositoryFilePath = filename.getName() + "."+ AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_FIELDOFACTIVITYANNOTATIONS;
		
		if (version.getIECRepository()!=null)
			saveEmfModelToResource(version.getIECRepository(), 
					targetDirectoryPath.toAbsolutePath().toString(), repositoryfilePath, resourceSet);		
		if (version.getConfiguration()!=null)
			saveEmfModelToResource(version.getConfiguration(), 
					targetDirectoryPath.toAbsolutePath().toString(), configurationFilePath, resourceSet);		
		if (version.getModificationMarkRepository()!=null)
			saveEmfModelToResource(version.getModificationMarkRepository(), 
					targetDirectoryPath.toAbsolutePath().toString(), internalIECModFilePath, resourceSet);		
		if (version.getFieldOfActivityRepository()!=null)
			saveEmfModelToResource(version.getFieldOfActivityRepository(), targetDirectoryPath
					.toAbsolutePath().toString(), iecFieldOfActivityRepositoryFilePath, resourceSet);
		if (version.getHmiRepository()!=null)
			saveEmfModelToResource(version.getHmiRepository(), 
					targetDirectoryPath.toAbsolutePath().toString(), hmiRepositoryFilePath, resourceSet);
		if (version.getHmiModificationRepository()!=null)
			saveEmfModelToResource(version.getHmiModificationRepository(), targetDirectoryPath
					.toAbsolutePath().toString(), hmiModificationMarksFilePath, resourceSet);
	}
