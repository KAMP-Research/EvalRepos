	...
	private static String customizeBusComponent(ModifyComponent<Component> modification) {
		if (modification.getAffectedElement() instanceof BusBox) {
			...
			for (SignalInterface si : busBox.getSignalinterfaces()) {
				custom += getOutputString(new Id(busBox.getSignalinterface_box().getId()), new Id(si.getId()));
			}
			...
			for (SignalInterface si : busMaster.getSignalinterfaces()) {
				custom += getOutputString(new Id(busMaster.getSignalinterface_controller().getId()), new Id(si.getId())) + "\n";
			}
			return custom;
			...
			return getOutputString(new Id(busCable.getSignalPlug1().getId()), new Id(busCable.getSignalPlug2().getId()));
		}
		return "";
	}
