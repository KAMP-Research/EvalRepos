...
public class APSArchitectureVersionPersistency extends AbstractArchitectureVersionPersistency<APSArchitectureVersion> {
	...
	@Override
	public APSArchitectureVersion load(Path folderpath, File file, Version versionname) {
		ResourceSet loadResourceSet = new ResourceSetImpl();
		String fieldOfActivityRepositoryFilePath = file.getName() + "." + FILEEXTENSION_FIELDOFACTIVITYANNOTATIONS;
		String internalModFilePath = file.getName() + "." + FILEEXTENSION_MODIFICATIONMARKS;
		String xppufilePath = file.getName() + "." + FILEEXTENSION_APS;

		String internalIECFieldOfActivityFilePath = file.getName() + "." + AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_FIELDOFACTIVITYANNOTATIONS;
		String internalIecRepositoryFilePath = file.getName() + "." + AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_REPOSITORY;
		String internalConfigurationFilePath = file.getName() + "." + AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_CONFIGURATION;
		String internalIECModFilePath = file.getName() + "." + AbstractKAMP4IECArchitectureVersionPersistency.FILEEXTENSION_MODIFICATIONMARK;
		
		String internalHMIPath = file.getName() + "." + FILEEXTENSION_HMI;
		String internalHMIModificationsPath = 
				file.getName() + "." + FILEEXTENSION_HMI_MODIFICATIONMARKS;

		archParams.name = versionname.getVersionName();
		archParams.fieldOfActivityRepository = (FieldOfActivityAnnotationRepository) loadEmfModelFromResource(
				folderpath.toString(), fieldOfActivityRepositoryFilePath, loadResourceSet);
		archParams.modificationMarkRepository = (KAMP4aPSModificationRepository) loadEmfModelFromResource(
				folderpath.toString(), internalModFilePath, loadResourceSet);
		archParams.aPSPlant = (Plant) loadEmfModelFromResource(folderpath.toString(), xppufilePath, loadResourceSet);
		archParams.deploymentContextRepository = (DeploymentContextRepository) loadEmfModelFromResource(folderpath.toString(),
				internalModFilePath, loadResourceSet);

		archParams.iecFieldOfActivityRepository = (IECFieldOfActivityAnnotationsRepository)loadEmfModelFromResource(folderpath.toString(), internalIECFieldOfActivityFilePath, loadResourceSet);
		archParams.iecRepository = (Repository)loadEmfModelFromResource(folderpath.toString(), internalIecRepositoryFilePath, loadResourceSet);
		archParams.configuration = (Configuration)loadEmfModelFromResource(folderpath.toString(), internalConfigurationFilePath, loadResourceSet);
		archParams.iecModificationMarkRepository = (IECModificationRepository)loadEmfModelFromResource(folderpath.toString(), internalIECModFilePath, loadResourceSet);
		
		archParams.hmiRepository = (edu.kit.ipd.sdq.kamp4hmi.model.Kamp4hmiModel.Repository)loadEmfModelFromResource(folderpath.toString(), internalHMIPath, loadResourceSet);
		archParams.hmiModificationMarksRepository = (HMIModificationMarksRepository)loadEmfModelFromResource(folderpath.toString(), internalHMIModificationsPath, loadResourceSet);

		return new APSArchitectureVersion(archParams);
	}

	@Override
	public APSArchitectureVersion load(IContainer folder, Version versionname) {
		ResourceSet loadResourceSet = new ResourceSetImpl();
