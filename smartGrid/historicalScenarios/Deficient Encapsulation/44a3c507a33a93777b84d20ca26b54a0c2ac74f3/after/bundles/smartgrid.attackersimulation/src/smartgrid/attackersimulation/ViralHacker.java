...
public class ViralHacker implements IAttackerSimulation {
     ...
     */
    @Override
    public ScenarioResult run(new SmartGridTopologyScenarioResult scenario) {
        if (!this.initDone) {
            throw new IllegalStateException("ViralHacker not initialization. Run init()");
        }
        if (this.firstRun ) {
            if (this.rootNode.equals(Constants.DEFAULT_ROOT_NODE_ID) && getHackedNodes(scenario.getResult()).size() == 0) {
                this.rootNode = ScenarioModelHelper.selectRandomRoot(this.ignoreLogicalConnections, scenario.getResult());
            } else if (this.rootNode.equals(Constants.DEFAULT_ROOT_NODE_ID) && getHackedNodes(scenario.getResult()).size() != 0) {
            	this.rootNode = String.valueOf(getHackedNodes(scenario.getResult()).get(0).getOwner().getId());
            }
            final var rootState = ScenarioModelHelper.findEntityOnStateFromID(this.rootNode, scenario.getResult());
        ...
        case STANDARD_HACKING:
            this.standardHacking(scenario.getTopo(), scenario.getResult());
            break;
        case FULLY_MESHED_HACKING:
            this.fullyMeshedHacking(scenario.getResult());
            break;
        default:
