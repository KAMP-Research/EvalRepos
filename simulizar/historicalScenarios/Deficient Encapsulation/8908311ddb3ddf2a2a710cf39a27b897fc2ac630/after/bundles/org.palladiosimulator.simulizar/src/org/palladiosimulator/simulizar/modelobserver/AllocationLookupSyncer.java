...
public class AllocationLookupSyncer extends AbstractModelObserver<Allocation>
    ...
    private final Map<String, AbstractSimulatedResourceContainer> simulatedContainerStorage =
            new HashMap<>();
    ...
    public void initialize(AbstractSimuLizarRuntimeState runtimeState) {
        ...
        addInitialAllocations(
                runtimeState.getPCMPartitionManager().getGlobalPCMModel().getAllocation());
    }
     ...
     */
    protected void addAssemblyAllocation(AssemblyContext ctx, 
            CtxHierarchy ctxHierarchy,
            AbstractSimulatedResourceContainer container) {
        if (ctxHierarchy.isEmpty()) {
            simulatedContainerStorage.put(ctx.getId(), container);
        } else {
            var newHierarchy = ctxHierarchy;
            newHierarchy.push(ctx);
            simulatedContainerStorage.put(new FQComponentID(newHierarchy).getFQIDString(),
                    container);
        }
        ...
        if (component instanceof CompositeComponent) {
            ...
            for (var compCtx : composite.getAssemblyContexts__ComposedStructure()) {
                addAssemblyAllocation(compCtx, ctxHierarchy, container);
            }
        }
    }
     ...
     *                     CompositeComponents. Provide an empty list if none.
     */
    protected void removeAssemblyAllocation(AssemblyContext ctx, CtxHierarchy ctxHierarchy) {
    ...
    private void processNotification(Notification notification, Function<Notification, Object> extractor,
        ...
        assert notification.getFeature() == AllocationPackage.L
            terals.ALLOCATION__ALLOCATION_CONTEXTS_AL 
        Object feature = no
                tification.getNewValue();
