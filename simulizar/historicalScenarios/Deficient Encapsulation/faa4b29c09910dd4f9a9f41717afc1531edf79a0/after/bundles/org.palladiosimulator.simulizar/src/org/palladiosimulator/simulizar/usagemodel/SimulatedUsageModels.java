...
public class SimulatedUsageModels {
    ...
    private final Map<ClosedWorkload, de.uka.ipd.sdq.simucomframework.usage.ClosedWorkload> closedWorkloads =
            new HashMap<ClosedWorkload, de.uka.ipd.sdq.simucomframework.usage.ClosedWorkload>();
    private final Map<OpenWorkload, de.uka.ipd.sdq.simucomframework.usage.OpenWorkload> openWorkloads =
            new HashMap<OpenWorkload, de.uka.ipd.sdq.simucomframework.usage.OpenWorkload>();
    ...
    public IWorkloadDriver[] createWorkloadDrivers() {
        final EList<UsageScenario> usageScenarios = this.rootContext.getPCMPartitionManager()
                .getGlobalPCMModel().getUsageModel().getUsageScenario_UsageModel();
    ...
    public IWorkloadDriver createAndAddWorkloadDriver(final UsageScenario usageScenario) {
        ...
        if (workload.eClass() == UsagemodelPackage.eINSTANCE.getClosedWorkload()) {
            final de.uka.ipd.sdq.simucomframework.usage.ClosedWorkload driver =
                    this.createClosedWorkloadDriver(
                            new WorkloadUseageScenario(workload, usageScenario));
            ...
            final de.uka.ipd.sdq.simucomframework.usage.OpenWorkload driver = this
                    .createOpenWorkloadDriver(new WorkloadUseageScenario(workload, usageScenario));
    ...
    private de.uka.ipd.sdq.simucomframework.usage.ClosedWorkload createClosedWorkloadDriver(
            WorkloadUseageScenario wlu) {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Create workload driver for ClosedWorkload: " + wlu.getWorkload());
        }
        final ClosedWorkload closedWorkload = (ClosedWorkload) wlu.getWorkload();

        final IClosedWorkloadUserFactory userFactory =
                new ClosedWorkloadUserFactory(this.rootContext.getModel(),
                        closedWorkload.getThinkTime_ClosedWorkload().getSpecification(),
                        wlu.getUsageScenario()) {

                    @Override
                    public IScenarioRunner createScenarioRunner() {
                        return SimulatedUsageModels.this.getScenarioRunner(wlu.getUsageScenario());
                    }
                };

        // create workload driver by using given factory
        return new de.uka.ipd.sdq.simucomframework.usage.ClosedWorkload(userFactory,
                closedWorkload.getPopulation());
    }

    private de.uka.ipd.sdq.simucomframework.usage.OpenWorkload createOpenWorkloadDriver(
            WorkloadUseageScenario wlu) {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Create workload driver for OpenWorkload: " + wlu.getWorkload());
        }
        final OpenWorkload openWorkload = (OpenWorkload) wlu.getWorkload();

        final IUserFactory userFactory =
                new OpenWorkloadUserFactory(this.rootContext.getModel(), wlu.getUsageScenario()) {
        ...
        // create workload driver by using given factory
        return new de.uka.ipd.sdq.simucomframework.usage.OpenWorkload(this.rootContext.getModel(),
                userFactory, openWorkload.getInterArrivalTime_OpenWorkload().getSpecification());
    }
        ...
        return new IScenarioRunner() {
            ...
            public void scenarioRunner(final SimuComSimProcess thread) {
                ...
                final UsageModel usageModel =
                        newContext.getPCMPartitionManager().getLocalPCMModel().getUsageModel();
                ...
                usageModel.getUsageScenario_UsageModel().stream()
                        .filter(sc -> sc.getId().equals(scenario.getId())).findAny()
                        .ifPresent(sc -> {
    ...
    public de.uka.ipd.sdq.simucomframework.usage.OpenWorkload getOpenWorkloadDriver(
            final OpenWorkload openWorkload) {
