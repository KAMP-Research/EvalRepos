...
public class ResourceUtil {
	...
	public static Resource createResourceOf(String fileExt, ResourceFactoryImpl resourceFactory,
		...
		URI rmModelUri = URI.createURI(construct(new FileExt(fileExt), Path.of(storingLocation)));
	...
	private static String construct(FileExt fileExt, Path storingLocation) {
		storingLocation = storingLocation.getParent();
		return Path.of(storingLocation.toString(), DEFAULT_FILE_NAME + "." + fileExt.getFileExt()).toString();
	}
	...
	private static URI defaultFileURI() {
		...
		try {
			...
			LOGGER.info(String.format(
					"The temporary file for the runtime measurement model could not be created: %s",
					ex.getMessage()));
		}
